...
lcvgc デーモンに MIDI 入力デバイス（鍵盤等）を繋ぎ、演奏した音をそのまま DSL 文法でバッファに書き込めます。

```vim
" 入力ポートを選んで購読を開始（port 省略時は vim.ui.select で選択）
...
- ノーマルモードの `<C-e><C-r>` で購読のオン/オフをトグルできます。
- 鍵盤を弾くと、デーモンが `midi_in_event` をプッシュ送信し、1発音が1トークン（`name:octave` 形式、例: MIDI 60 = `c:4`）としてカーソル位置にスペース区切りで挿入されます（例: `c:4 e:4 g:4`）。
- NoteOff・velocity 0 の NoteOn・CC・Program・System Real-Time はイベント化されません（鍵盤を弾いた発音だけが届きます）。
- 1接続につき購読は1つで、再購読すると張り替わります。Neovim 終了時に自動で解除されます。
